...
def remove_H(data):
    for models in data:     
        ...
        data[models]['amino_acid'] = data[models]['amino_acid'][non_H_atom]
        
    print('Removed H atoms from', list(data.keys()))
    return data
...
def reorder_atoms(data):
    ...
    print('Reordered',list(data.keys()),'atoms :', data[models]['atom'][0:10])
    return data
...
def get_backbone(data, reorder=True):
    ...
    print('Backbone atoms extracted for ',list(data.keys()))
    return data
...
def trim_resid(data, start = 25, end = 285):
    ...
    for models in backbone: 
        ...
        backbone[models]['amino_acid'] = backbone[models]['amino_acid'][keep_resid]
    print('Resid trimed from',start, 'to', end, 'for', list(data.keys()))
    return backbone
...
def tmalign_data(data, align=None, rotate_data = None, skip_data=None):
    remove = []
    if align == None :
        align = list(data.keys())[0]
    for models in data: 
        if models == align: 
            continue 
        if models in skip_data: 
            print(f'{models} skipped. via skip_data')
            continue
...
def dict_to_longdf(data):
    """
    This chunk takes the dictionary form of xyz data and transforms it
    Output to a long form of dataframe instead 
    """
    long_df = pd.DataFrame()
    for Or in data:
        row_data_list = []
        row_column_list = []
        for i in enumerate(set(data[Or]['resid'])):
            row_data_list.append(list(data[Or]['coord'][i[0]]))
            row_column_list.append(["{}_{}_x".format(data[Or]['resid'][i[0]], 
                                                 data[Or]['atom'][i[0]]),
                                "{}_{}_y".format(data[Or]['resid'][i[0]], 
                                                 data[Or]['atom'][i[0]]),
                                "{}_{}_z".format(data[Or]['resid'][i[0]], 
                                                 data[Or]['atom'][i[0]])])
        # Flatten the list here to join together in a row 
        row_data_list = list(itertools.chain(*row_data_list))
        row_column_list = list(itertools.chain(*row_column_list))
        # Join the rows and columns into one single row of data frame
        row_data = pd.DataFrame([row_data_list], columns=row_column_list)
        row_data.index = [Or]
        # Join the Rows to the final dataframe 
        long_df = pd.concat([long_df,row_data])
