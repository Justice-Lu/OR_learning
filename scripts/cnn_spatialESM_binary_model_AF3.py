...
Cbc_cav_coords = pd.read_pickle('/mnt/data2/Justice/OR_learning/files/binding_cavity/AF3_dict_Cbc_cav_coords.pkl')
Cbc_res_coords = pd.read_pickle('/mnt/data2/Justice/OR_learning/files/binding_cavity/AF3_dict_Cbc_res_coords.pkl')
...
class ORLigandIndexDataset(Dataset):
    def __init__(self, pairs_df, voxel_tensor, ligand_fp_tensor, 
                 or_index_map, ligand_index_map, augment=False):
        ...
        for _, row in pairs_df.iterrows():
            ...
            or_id_matches = [_key for _key in or_index_map.keys() if _key.split('_')[0].lower() == or_id.lower()]
            if (len(or_id_matches) > 0) and (cid in ligand_index_map):
                for _or_id in or_id_matches: 
                    self.indices.append((or_index_map[_or_id], ligand_index_map[cid]))
                    self.targets.append(1.0 if row['activation_zscore'] >= 2 else 0.0)
...
def create_balanced_datasets(ps6_df, voxel_tensor, ligand_fp_tensor,
                             ...
                             val_size=0.1, seed=None):

    np.random.seed(seed)

    # Positive and negative sets
    positive_df = ps6_df[ps6_df['activation_zscore'] >= 2]
    negative_df = ps6_df[(ps6_df['activation_zscore'] < 0.5) & 
                         (ps6_df['activation_zscore'] > -0.5) ]
    ...
    # Clamp counts to available data
    n_pos = min(n_pos, len(positive_df))
    n_neg = max(0, min(n_neg, len(negative_df)))

    # Sample
    pos_samples = positive_df.sample(n=n_pos, random_state=seed)
    neg_samples = negative_df.sample(n=n_neg, random_state=seed)
    
    # Combine & shuffle
    full_df = pd.concat([pos_samples, neg_samples]).sample(frac=1.0, random_state=seed)

    # === 2. Split into train / validation ===
    train_df, val_df = train_test_split(
        full_df,
        test_size=val_size,
        stratify=(full_df['activation_zscore'] >= 2),
        random_state=seed
    )
    ...
    print("Positives in full_df:", sum(full_df['activation_zscore'] >= 2))
    print("Negatives in full_df:", sum((full_df['activation_zscore'] < 0.5) & (full_df['activation_zscore'] > -0.5)))
    print("Train split counts:",
        sum(train_df['activation_zscore'] >= 2),
        sum((train_df['activation_zscore'] < 0.5) & (train_df['activation_zscore'] > -0.5)))
    print("Val split counts:",
        sum(val_df['activation_zscore'] >= 2),
        sum((val_df['activation_zscore'] < 0.5) & (val_df['activation_zscore'] > -0.5)))
...
def main():
    ...
    # Dynamically add overrides for each param_choice key
    for key in param_choices.keys():
        parser.add_argument(f"--{key}", type=str, nargs="+", help=f"Override values for {key}")
    ...
    if args.param_dir: 
        ...
        # Convert dicts to tuples of values for consistency
        param_list = [list(d.values()) for d in param_list]
    else: 
    ...
    for i, param_values in enumerate(sampled_params):
        ...
        train_dataset, val_dataset = create_balanced_datasets(
            ...
            total_train_samples=param_dict['total_sample'], positive_weight=param_dict['pos_ratio'], seed=i
        )
