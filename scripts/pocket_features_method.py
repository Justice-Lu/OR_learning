...
pocket_features.py
 By:
   Brian Coventry (bcov@uw.edu)
   Derrick Hicks (drhicks1@uw.edu)
 2022 Institute for Protein Design IDP

Cite:
  De novo design of protein homodimers containing tunable symmetric protein pockets PNAS 2022

 Finds a single large pocket in your protein using a variety of methods and reports the volume as well
  as the dimensions of the pocket

 pocket_volume.sc:
   pocket_volume -- the volume in Angstroms ^ 3
   dimension_1-3 -- the maximal length along the longest dimension, 2nd longest, 3rd longest. All orthogonal
   I1-3 -- the moment of inertia along the dimensions
   I/I -- Divisions of the moments of inertia. Related to the shape of the pocket


Packages:
   npose:
      available on pip `pip install npose`
      also available here: https://github.com/bcov77/npose
        either pip install the git clone or add it to sys.path.append() below

   numba:
      conda install numba

   atomic_depth:
      available here https://github.com/bcov77/atomic_depth
        git clone ...
        cd atomic_depth/
        pip install ./
'''
